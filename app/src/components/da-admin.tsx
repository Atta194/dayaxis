...
export default function Admin() {
  ...
  const stat = (label: string, num: number | string, tone = "brand") => (
  ...
  return (
    <div>
      ...
      <div className="grid mt3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))" }}>
        {stat("Accounts", stats?.accounts ?? "…", "brand")}
        {stat("Homes", stats?.homes ?? "…", "brand")}
        {stat("Tasks", stats?.tasks ?? "…", "gold")}
        {stat("Workers", stats?.workers ?? "…", "brand")}
        {stat(t("pending_queue"), stats?.pending ?? "…", "accent")}
        {stat("Reviews", stats?.reviews ?? "…")}
        {stat(t("feedback"), `${stats?.feedback_avg ?? "…"} ★ (${stats?.feedback ?? 0})`, "brand")}
        {stat(t("sponsor_stats"), stats?.ads_clicks ?? "…", "gold")}
        {stat(t("plan_active"), stats?.active_subs ?? "…", "brand")}
      </div>
